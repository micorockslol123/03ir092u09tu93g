...
for i,v in pairs(mas:GetChildren()) do
	v.Parent = game:GetService("Players").LocalPlayer.Character["Cat"]
...
local function CameraShake(Times, Power)
...
local function chatfunc(text,color,typet,font,timeex)
...
local function CreateSpecialGlowMesh(parent,meshid,x1,y1,z1)
local mesh = Instance.new("SpecialMesh",parent)
mesh.MeshType = "FileMesh"
mesh.MeshId = meshid
...
local function lerp(object, newCFrame, alpha)
...
local function Damagefunc(Part, hit, minim, maxim, knockback, Type, Property, Delay, HitSound, HitPitch)

end
local function ShowDamage(Pos, Text, Time, Color)

end
local function MagniDamage(Part, magni, mindam, maxdam, knock, Type,Sound)
...
local function sphere2(bonuspeed,type,pos,scale,value,value2,value3,color,color3)
local type = type
local rng = Instance.new("Part", char)
        rng.Anchored = true
        rng.BrickColor = color
...
local function block(bonuspeed,type,pos,scale,value,value2,value3,color,color3)
local type = type
local rng = Instance.new("Part", char)
        rng.Anchored = true
        rng.BrickColor = color
...
local function sphereMK(bonuspeed,FastSpeed,type,pos,x1,y1,z1,value,color,color3,outerpos)
local type = type
local rng = Instance.new("Part", char)
        rng.Anchored = true
        rng.BrickColor = color
...
local function waveEff(bonuspeed,type,typeoftrans,pos,scale,value,value2,color)
local type = type
local rng = Instance.new("Part", char)
        rng.Anchored = true
        rng.BrickColor = color
...
local function FindNearestTorso(Position, Distance, SinglePlayer)
...
local function eightbitmegablade()
attack = true
hum.WalkSpeed = 0
hum.JumpPower = 0
...
table.insert(_G.EventTable,plr.Chatted:connect(function(message)
