...
local function SpecialTrace(what)
...
local function Tween(obj,props,time,easing,direction,repeats,backwards)
    local info = TweenInfo.new(time or .5, easing or Enum.EasingStyle.Quad, direction or Enum.EasingDirection.Out, repeats or 0, backwards or false)
    local tween = game:service'TweenService':Create(obj, info, props)
...
local function TargetSelect(person)
...
local Laser = function(brickcolor, reflect, cframe, x1, y1, z1, x3, y3, z3, delay)
...
local shoot = function(mouse, aoe , partt, SpreadAmount, multiply)
	...
	local Laser2 = function(brickcolor, reflect, cframe, x1, y1, z1, x3, y3, z3, delay)
...
local shoot2 = function(mouse, aoe , partt, SpreadAmount, multiply)
...
local function ShowDamage(Pos, Text, Time, Color, Color2)

end

local function MagnitudeDamage(Part, Magnitude, MinimumDamage, MaximumDamage, KnockBack, Type, HitSound, HitPitch)
...
local Halfhed=CFuncs.Part.Create(m,Enum.Material.Neon,0,0,"Really black","Halfhed",Vector3.new(2, 1, 1))
local HalfhedWeld=CFuncs.Weld.Create(m,Character["Head"],Halfhed,CFrame.new(0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, 1),CFrame.new(0.000106811523, -0.184874296, 3.43322754e-05, -1.00000834, 0.000114514238, -6.94826713e-06, 0.000117048308, 1, 1.29755635e-05, 7.1044451e-06, 1.3261616e-05, -1))
